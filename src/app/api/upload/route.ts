...
// WhatsApp chat analizi için gerekli yardımcı fonksiyonlar
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Dosya içeriğini hemen analiz edelim
    // Sayaçlar
    ...
    const emoji_counts_sorted = Object.entries(emoji_counts).sort((a,b)=>b[1]-a[1]).slice(0,10).reduce((acc,[e,c])=>(acc[e]=c,acc),{} as Record<string,number>);    // Dosya ismi oluşturma (sadece frontend için referans)
    const filename = `memory_${Date.now()}_${file.name}`;
      ...
      return NextResponse.json({ error: 'Dosya içeriği işlenirken bir hata oluştu. Dosya formatını kontrol ediniz.' }, { status: 400 });
    }
  } catch (error) {
